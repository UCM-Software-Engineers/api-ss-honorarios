...
export const getTerms = async (req, res) => {
  try {
    const [rows] = await connection.query('SELECT * FROM honorario_termino_referencia');
    res.status(200).json(rows);
  } catch (error) {
    res.status(500).json({ error: 'Error al obtener los términos de referencia' });
  }
};

export const createTerm = async (req, res) => {
    const termData = req.body;  
    const missingFields = requiredFields.filter(field => !termData.hasOwnProperty(field));
    ...
    try {
        const [rows] = await connection.query('SELECT MAX(id_termino_referencia) AS maxId FROM honorario_termino_referencia');
        const maxId = rows[0].maxId || 0; 
        const newId = maxId + 1;
        termData.id_termino_referencia = newId;
    
        await connection.query('INSERT INTO honorario_termino_referencia SET ?', termData);
...
export const deleteTerm = async (req, res) => {
    ...
    try {
      const [rows] = await connection.query('SELECT * FROM honorario_termino_referencia WHERE id_termino_referencia = ?', [id]);
      if (rows.length === 0) {
        return res.status(404).json({ error: 'Término de referencia no encontrado' });
      }
  
      await connection.query('DELETE FROM honorario_termino_referencia WHERE id_termino_referencia = ?', [id]);
...
export const updateTerm = async (req, res) => {
    ...
    try {
      const [rows] = await connection.query('SELECT * FROM honorario_termino_referencia WHERE id_termino_referencia = ?', [id_termino_referencia]);
      if (rows.length === 0) {
        return res.status(404).json({ error: 'Término de referencia no encontrado' });
      }
